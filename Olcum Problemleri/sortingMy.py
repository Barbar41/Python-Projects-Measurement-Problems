...
df = pd.read_csv("Olcum Problemleri/datasets/product_sorting.csv")
print(df.shape)
df.head(10)
...
# Ratinge göre puanı en tepede olacak sekılde bır sıralayalım
...
####################
# Satın alma sayısına göre sıralayalım
df.sort_values("purchase_count", ascending=False).head(20)

# Yorum sayısına göre sıralayalım
...
# Üç faktörü aynı anda bir arada bulundurmak.Buna göre yakalamak.
# Hepsini aynı ölçeğe getirerek bir yaklaşalım
...
# Bu işlemin benzerini comment_count için yapmalıyız
...
# Bu değişkenler aynı cinsden öyle ise ortalamalarını alabiliriz.
# Ağırlıklandırabiliriz ve böylece bir skor alabiliriz.(Ağırlıklandırma ile oluşan)
...
# Puanların dağılım bilgisini kullanarak agırlıklı bır sekılde olasılıksal bir ortalama hesabı yapalım
...
# Yeni oluşturdugumuz bar_score gore sıralama yapalım.
# bar_score bize sadece ratinglere(puanlamalara) odaklanarak bir sıralama saglıyor.
df.sort_values("bar_score", ascending=False).head(20)

# Kurs isimlerinden indexe göre getirelim
...
# Veri bilimi keyword ile aramalarda gelecek kurslar
...
df = pd.read_csv("Olcum Problemleri/datasets/movies_metadata.csv",
...
# Filmleri ortalama puanlarına göre sıraladığımızda
df.sort_values("vote_average", ascending=False).head(20)

# Oy sayılarını kucukten buyuge sıralandıktan sonra bunların ceyrek degerlerı bılgısı.
# (Nereden filtreleme yapabileceğimizi kontrol ediyoruz)
df["vote_count"].describe([0.10, 0.25, 0.50, 0.70, 0.80, 0.90, 0.95, 0.99]).T

# Koşul olarak 400 üzeri üzeri olan oy sayısı olanalrı sartı ıle sıraladık.
df[df["vote_count"] > 400].sort_values("vote_average", ascending=False).head(20)

from sklearn.preprocessing import MinMaxScaler

# Etkilerini gördüğümüz şekilde birlikte kullanım yapalım.
# vote_count değişkenini standartlaştıralım.Skorlaştıralım.
df["vote_count_score"] = MinMaxScaler(feature_range=(1, 10)).fit(df[["vote_count"]]). \
                         transform(df[["vote_count"]])

# Bir average_count_score değişkeni türetelim.
df["average_count_score"] = df["vote_average"] * df["vote_count_score"]

# Bu değişkene göre sıralama yapalım
...
# Oyların dagılımını alalım
df = pd.read_csv("Olcum Problemleri/datasets/imdb_ratings.csv")
df = df.iloc[0:, 1:]

# 250 film var bu sıralama ıcın bakalım
